...
import 'package:sba/src/widgets/nodata_widget.dart';
...
typedef PagedValueScrollViewIndexedWidgetBuilder<T> = Widget Function(
    BuildContext context, List<T> values, int index, WidgetRef ref);
...
class PagedValueListView<K, V, C extends PagingValueNotifier<K, V>>
  ...
  //           PagingValue<int, AppendixPendingModel>>

  final AutoDisposeStateNotifierProvider<C, PagingValue<K, V>> provider;
  ...
  @override
  ConsumerState<PagedValueListView<K, V, C>> createState() =>
      _PagedValueListViewState<K, V, C>();
}

class _PagedValueListViewState<K, V, C extends PagingValueNotifier<K, V>>
    extends ConsumerState<PagedValueListView<K, V, C>> {
  AutoDisposeStateNotifierProvider<C, PagingValue<K, V>> get _provider =>
      widget.provider;
  ...
  @override
  void didUpdateWidget(covariant PagedValueListView<K, V, C> oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (ref.read(widget.provider) != ref.read(oldWidget.provider)) {
      _hasRequestedNextPage = false;
      ref.read(widget.provider.notifier).doInitialLoad();
    }
  }
    ...
    return state.when(
      (items, nextPageKey, error) {
        ...
        return RefreshIndicator.adaptive(
          onRefresh: () {
            return ref.read(_provider.notifier).refresh();
          },
          child: ListView.separated(
            ...
            separatorBuilder: (context, index) =>
                widget.separatorBuilder(context, items, index, ref),
            itemBuilder: (context, index) {
              ...
              if (index == items.length) {
                if (error != null) {
                  return const Text('Lỗi load more');
              ...
              return widget.itemBuilder(context, items, index, ref);
            },
          ),
        );
      },
