...
import 'package:sba/src/widgets/custom_mergeable_material.dart' as m;
import 'package:sba/src/widgets/custom_expand_icon.dart' as c;
...
class _ExpansionPanelListState extends State<ExpansionPanelList> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<m.MergeableMaterialItem> items = <m.MergeableMaterialItem>[];

    for (int index = 0; index < widget.children.length; index += 1) {
      if (_isChildExpanded(index) &&
          index != 0 &&
          !_isChildExpanded(index - 1)) {
        items.add(m.MaterialGap(
      ...
      Widget header = Card(
        ...
        shape: RoundedRectangleBorder(
            borderRadius:
                BorderRadius.circular(_isChildExpanded(index) ? 8 : 0)),
        elevation: _isChildExpanded(index) ? 8 : 0,
        child: Stack(
          children: <Widget>[
            AnimatedContainer(
