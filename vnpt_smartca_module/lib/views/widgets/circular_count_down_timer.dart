...
class CircularCountDownTimerState extends State<CircularCountDownTimer>
  ...
  void _setController() {
    ...
    if (widget.initialDuration > 0 && widget.autoStart) {
      _controller.value = widget.isReverse
          ? 1 - (widget.initialDuration / widget.duration)
          : (widget.initialDuration / widget.duration);
...
class CountDownController {
  ...
  int _initialDuration = 0, _duration = 0;
  ...
  void restart({required int duration}) {
    _state._controller.duration = Duration(seconds: duration);
    if (_isReverse) {
